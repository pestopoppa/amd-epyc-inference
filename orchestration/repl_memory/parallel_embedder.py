...
class ParallelEmbedderClient:
    ...
    def embed_sync(self, text: str) -> np.ndarray:
        ...
        if loop is not None:
            ...
            future = self._get_sync_pool().submit(asyncio.run, self.embed_async(text))
    ...
    def embed_batch_sync(self, texts: list[str]) -> np.ndarray:
        ...
        if loop is not None:
            future = self._get_sync_pool().submit(asyncio.run, self.embed_batch_async(texts))
            return future.result()
        else:
    ...
    def health_check_all_sync(self) -> dict[str, bool]:
        ...
        if loop is not None:
            future = self._get_sync_pool().submit(asyncio.run, self.health_check_all())
            return future.result()
        else:
    ...
    def close_sync(self) -> None:
        ...
        if self._http_client is not None:
            ...
            if loop is not None:
                self._get_sync_pool().submit(asyncio.run, self.close()).result()
            else:
