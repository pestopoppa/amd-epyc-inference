...
summary() {
    ...
    local sessions=$(grep -o '"session":"[^"]*"' "$LOG_FILE" | sort -u | wc -l)
    local errors=$(grep -c '"level":"ERROR"' "$LOG_FILE" || echo 0)
    local warnings=$(grep -c '"level":"WARN"' "$LOG_FILE" || echo 0)
    local commands=$(grep -c '"cat":"CMD_INTENT"' "$LOG_FILE" || echo 0)
    local tasks=$(grep -c '"cat":"TASK_START"' "$LOG_FILE" || echo 0)
    ...
    grep '"cat":"SESSION_START"' "$LOG_FILE" | tail -5 | while read -r line; do
        ...
        local session=$(echo "$line" | jq -r '.session' 2>/dev/null || echo "?")
        local msg=$(echo "$line" | jq -r '.msg' 2>/dev/null || echo "?")
        echo "  $ts - $session: $msg"
    done
    echo ""
...
detect_loops() {
    ...
    local sessions=$(grep -o '"session":"[^"]*"' "$LOG_FILE" | sort -u | sed 's/"session":"//;s/"//')
    
    for session in $sessions; do
        local repeated=$(grep "\"session\":\"$session\"" "$LOG_FILE" | \
            grep '"cat":"CMD_INTENT"' | \
            jq -r '.msg' 2>/dev/null | \
            sort | uniq -c | sort -rn | \
...
show_errors() {
    ...
    grep '"level":"ERROR"' "$LOG_FILE" | while read -r line; do
        ...
        local session=$(echo "$line" | jq -r '.session' 2>/dev/null || echo "?")
        local msg=$(echo "$line" | jq -r '.msg' 2>/dev/null || echo "?")
        local details=$(echo "$line" | jq -r '.details' 2>/dev/null || echo "")
...
list_sessions() {
    ...
    grep '"cat":"SESSION_START"' "$LOG_FILE" | while read -r line; do
        ...
        local cmd_count=$(grep "\"session\":\"$session\"" "$LOG_FILE" | grep -c '"cat":"CMD_INTENT"' || echo 0)
        local err_count=$(grep "\"session\":\"$session\"" "$LOG_FILE" | grep -c '"level":"ERROR"' || echo 0)
...
show_session() {
    ...
    grep "\"session\":\"$session_id\"" "$LOG_FILE" | while read -r line; do
        local ts=$(echo "$line" | jq -r '.ts' 2>/dev/null | sed 's/T/ /' | cut -d'+' -f1)
        local level=$(echo "$line" | jq -r '.level' 2>/dev/null || echo "?")
        local cat=$(echo "$line" | jq -r '.cat' 2>/dev/null || echo "?")
        local msg=$(echo "$line" | jq -r '.msg' 2>/dev/null || echo "?")
        local details=$(echo "$line" | jq -r '.details' 2>/dev/null || echo "")
