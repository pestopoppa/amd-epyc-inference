...
validate_draft_compatibility() {
    ...
    local test_acceptance=$(grep -oP 'accept\s+=\s+\K[\d.]+' "$validation_output" 2>/dev/null | tail -1 || echo "")
...
run_test() {
    ...
    if [[ "${BENCHMARK_SKIP_EXISTING:-true}" == "true" ]] && [[ -f "$output_file" ]]; then
        if grep -q "tokens per second" "$output_file" 2>/dev/null; then
            local existing_speed=$(grep "eval time" "$output_file" 2>/dev/null | grep -oP '\d+\.\d+(?= tokens per second)' | tail -1)
...
DRYRUN
    ...
    if is_spec_config "$config"; then
        ...
        if is_moe_lookup_config "$config"; then
            local exp_count=$(get_moe_from_lookup_combo "$config")
