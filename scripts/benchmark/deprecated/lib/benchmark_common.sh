...
init_benchmark_run() {
    ...
    # Get system info
    local llama_version=$(get_llama_version)
    local llama_commit=$(get_llama_commit)
    local hostname=$(hostname)
    local cpu_model=$(lscpu 2>/dev/null | grep 'Model name' | cut -d: -f2 | xargs || echo "unknown")
    local total_ram=$(free -h 2>/dev/null | grep Mem | awk '{print $2}' || echo "unknown")

    # Get model file hash (first 16 chars of sha256, or size if too slow)
    local model_size=$(stat -c%s "$model_path" 2>/dev/null || echo "0")
    local model_mtime=$(stat -c%Y "$model_path" 2>/dev/null || echo "0")
