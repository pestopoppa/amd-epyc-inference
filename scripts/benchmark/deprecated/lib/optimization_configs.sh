...
run_optimized_test() {
    ...
    if is_spec_config "$config"; then
        ...
        local k_value=$(get_spec_k "$config")
        ...
        # Prompt lookup
        local ngram=$(get_lookup_ngram "$config")
        timeout "$timeout_sec" OMP_NUM_THREADS=1 numactl --interleave=all \
            "$LLAMA_CLI" \
            -m "$model_path" \
        ...
        # Baseline or MoE
        local moe_override=$(get_moe_override "$config" "$model_arch")
        timeout "$timeout_sec" OMP_NUM_THREADS=1 numactl --interleave=all \
            "$LLAMA_COMPLETION" \
            -m "$model_path" \
            -t 96 -n "$max_tokens" --temp "$temperature" \
...
optimize_temperature() {
    ...
    test_temp() {
        ...
        if is_spec_config "$base_config"; then
            local k_value=$(get_spec_k "$base_config")
            ...
            local moe_override=$(get_moe_override "$base_config" "$model_arch")
            timeout 120 OMP_NUM_THREADS=1 numactl --interleave=all \
                "$LLAMA_COMPLETION" \
                -m "$model_path" \
                -t 96 -n "$max_tokens" --temp "$temp" \
    ...
    while (( $(echo "$high - $low > $precision" | bc -l) )) && [[ $iteration -lt $max_iterations ]]; do
        ...
        local speed_low=$(test_temp "$low")
        local speed_q1=$(test_temp "$q1")
        local speed_mid=$(test_temp "$mid")
        local speed_q3=$(test_temp "$q3")
        local speed_high=$(test_temp "$high")
...
optimize_temperature_quick() {
    ...
    for temp in "${TEMP_COARSE[@]}"; do
        ...
        local speed=$(extract_speed "$test_file")
