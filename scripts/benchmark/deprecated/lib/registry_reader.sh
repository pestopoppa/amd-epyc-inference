...
get_launch_flags() {
    local model_name="$1"
    local flags=$(registry_get "models.${model_name}.launch_config.flags[]")
    echo "$flags" | tr '\n' ' '
}
...
is_spec_viable() {
    local model_name="$1"
    local viable=$(registry_get "models.${model_name}.speculative_decoding.viable")
    [[ "$viable" == "true" ]]
}
...
build_llama_command() {
    ...
    local path=$(get_model_path "$model_name")
    local arch=$(get_model_arch "$model_name")
    local flags=$(get_launch_flags "$model_name")
    local moe_key=$(get_moe_override_key "$model_name")
...
build_spec_command() {
    ...
    local path=$(get_model_path "$model_name")
    local drafts=$(get_compatible_drafts "$model_name")
    local draft_path=$(echo "$drafts" | head -1)
...
resolve_model_path() {
    ...
    # Search for it
    local found=$(find "$MODEL_BASE" -name "$(basename "$partial")" -type f 2>/dev/null | head -1)
    if [[ -n "$found" ]]; then
        echo "$found"
        return
    fi
