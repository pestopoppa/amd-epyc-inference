...
is_tested() {
    ...
    local model_base=$(basename "$model" .gguf)
...
run_baseline() {
    local model="$1"
    local output="$TMP_DIR/baseline_$(basename "$model" .gguf).txt"
    ...
    local speed=$(grep "eval time" "$output" | grep -oP '[\d.]+(?= tokens per second)' | tail -1)
    echo "${speed:-0}"
}
...
run_speculative() {
    ...
    local k="$3"
    local temp="$4"
    local output="$TMP_DIR/spec_k${k}_t${temp}_$(basename "$target" .gguf).txt"
    ...
    local speed=$(grep "decoded" "$output" | grep -oP '[\d.]+(?= t/s)' | tail -1)
    local accept=$(grep "accept" "$output" | grep -oP '[\d.]+(?=%)' | tail -1)
...
run_lookup_moe_combo() {
    ...
    local arch="$2"
    local experts="$3"
    local output="$TMP_DIR/lookup_moe_exp${experts}_$(basename "$model" .gguf).txt"
...
run_quality_comparison() {
    ...
    local baseline_out="$TMP_DIR/quality_baseline_$(basename "$model" .gguf).txt"
    local opt_out="$TMP_DIR/quality_${opt_name}_$(basename "$model" .gguf).txt"
    ...
    local baseline_code=$(sed -n '/^assistant$/,/^>/p' "$baseline_out" | grep -v "^assistant$" | grep -v "^>")
    local opt_code=$(sed -n '/^assistant$/,/^>/p' "$opt_out" | grep -v "^assistant$" | grep -v "^>")
...
run_moe_experts() {
    ...
    local arch="$2"
    local experts="$3"
    local output="$TMP_DIR/moe_exp${experts}_$(basename "$model" .gguf).txt"
...
run_lookup() {
    local model="$1"
    local task="$2"
    local output="$TMP_DIR/lookup_${task}_$(basename "$model" .gguf).txt"
...
main() {
    ...
    for model in "${MODELS[@]}"; do
        ...
        local type_info=$(detect_model_type "$model")
        ...
        if is_in_research_report "$model" || is_tested "$model" "baseline" 0 0 0; then
            ...
            local speed=$(run_baseline "$model")
        ...
        for task in summarize code; do
            if ! is_tested "$model" "lookup_$task" 16 0 0; then
                ...
                local result=$(run_lookup "$model" "$task")
                local speed="${result%%:*}"
                local accept="${result##*:}"
            ...
            if [ -n "$draft" ]; then
                ...
                for temp in 0.3 0.5 0.7; do
                    if ! is_tested "$model" "speculative" 12 "$temp" 0; then
                        ...
                        local result=$(run_speculative "$model" "$draft" 12 "$temp")
                        local speed="${result%%:*}"
                        local accept="${result##*:}"
        ...
        if [ "$model_type" == "moe" ] && [ "$arch" != "unknown" ]; then
            for experts in 6 4 3; do
                if ! is_tested "$model" "moe_experts" 0 0 "$experts"; then
                    ...
                    local result=$(run_moe_experts "$model" "$arch" "$experts")
                    local speed="${result%%:*}"
            ...
            for experts in 4 3; do
                if ! is_tested "$model" "moe_lookup_combo" 16 0 "$experts"; then
                    ...
                    local result=$(run_lookup_moe_combo "$model" "$arch" "$experts")
                    local speed=$(echo "$result" | cut -d: -f1)
                    local accept=$(echo "$result" | cut -d: -f2)
                    local quality=$(echo "$result" | cut -d: -f3)
            ...
            if ! is_tested "$model" "quality_check" 0 0 4; then
                ...
                local quality=$(run_quality_comparison "$model" "--override-kv ${arch}.expert_used_count=int:4" "exp4")
