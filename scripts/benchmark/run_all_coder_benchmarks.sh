...
echo "=============================================="
echo ""
echo "Results in: /tmp/claude/coder_rubric_results/"
echo ""
...
for model_name in "${TESTED_MODELS[@]}"; do
    if [[ -v CODER_MODELS[$model_name] ]]; then
        ...
        for cfg in "${configs[@]}"; do
            echo ""
            echo "=== $model_name ($cfg) ==="
            for f in /tmp/claude/coder_rubric_results/${model_name}_${cfg}_*.txt 2>/dev/null; do
                if [[ -f "$f" ]]; then
    ...
    if [[ -v CODER_MODELS[$model_name] ]]; then
        ...
        if [[ "$arch" == "qwen3moe" ]]; then
            ...
            for test in t1_q1_factorial t1_q2_reverse_words t1_q3_stack t2_q1_palindrome t2_q2_async_fetch t2_q3_lru_cache t3_q1_ip_addresses t3_q2_rate_limiter t3_q3_bug_fix t3_q4_retry_decorator; do
                base_file="/tmp/claude/coder_rubric_results/${model_name}_baseline_${test}.txt"
                moe_file="/tmp/claude/coder_rubric_results/${model_name}_moe4_${test}.txt"
