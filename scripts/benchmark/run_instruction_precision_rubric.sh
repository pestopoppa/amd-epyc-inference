...
MODEL_ARCH="${3:-dense}"
OUTPUT_DIR="/tmp/claude/instruction_precision_rubric_results"
...
score_json_only() {
    ...
    local cleaned=$(echo "$response" | grep -v "^llama\|^load\|^print_info\|^common\|^sampler" | tr -d '\n' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')
...
score_count_items() {
    local response="$1"
    local expected="$2"
    local count=$(echo "$response" | grep -v "^llama\|^load\|^$" | grep -cE "^[0-9]+\.|^-|^•|^[A-Z]" || echo "0")
    # Also try counting non-empty lines as items
    local line_count=$(echo "$response" | grep -v "^llama\|^load\|^print_info\|^common\|^sampler\|^$" | grep -cE ".+" || echo "0")
...
score_forbidden_words() {
    local response="$1"
    local forbidden="$2"
    local cleaned=$(echo "$response" | grep -v "^llama\|^load\|^print_info" | tr '[:upper:]' '[:lower:]')
...
score_word_count() {
    ...
    local min="$2"
    local max="$3"
    local cleaned=$(echo "$response" | grep -v "^llama\|^load\|^print_info\|^common\|^sampler")
    local count=$(echo "$cleaned" | wc -w)
...
run_test() {
    ...
    echo "$prompt" > "/tmp/claude/ip_prompt.txt"

    timeout "$TIMEOUT" "$LLAMA_COMPLETION" \
        -m "$MODEL" \
        -t 96 -n 256 --temp 0.1 \
        $moe_override \
        -f "/tmp/claude/ip_prompt.txt" \
        > "$output_file" 2>&1 || true

    local speed=$(grep "eval time" "$output_file" | grep -oP '\d+\.\d+ tokens per second' | tail -1 || echo "N/A")
    local response=$(grep -v "^llama\|^load\|^print_info\|^common\|^sampler\|^generate\|^system_info\|^main:" "$output_file" | head -20)
...
"Explain photosynthesis in 2 sentences. Do NOT mention sunlight, sun, or solar." \
        "$CONFIG" "$MOE_OVERRIDE" "forbidden_words" "sunlight,sun,solar"

    run_test "t2_q2_word_limit" \
"Describe machine learning in exactly 20-25 words. Count carefully." \
        "$CONFIG" "$MOE_OVERRIDE" "word_count" "20:25"
...
- No additional text or punctuation" \
        "$CONFIG" "$MOE_OVERRIDE" "count_items" "5"
...
"What is 2+2? Reply with only the number, nothing else." \
        "$CONFIG" "$MOE_OVERRIDE" "exact_match" "4"
...
Do not explain or apologize." \
        "$CONFIG" "$MOE_OVERRIDE" "exact_match" "NONE"
...
- Does not use 'store' or 'storage'" \
        "$CONFIG" "$MOE_OVERRIDE" "word_count" "1:12"
    ...
    echo "Total: $((t1_pass + t2_pass + t3_pass))/11"
done
