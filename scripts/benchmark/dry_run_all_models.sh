...
test_model() {
    local model_path="$1"
    local model_name=$(basename "$(dirname "$model_path")")/$(basename "$model_path")
    ((TOTAL++))
    ...
    if timeout "$TIMEOUT_SECONDS" env OMP_NUM_THREADS=1 numactl --interleave=all \
        ...
        # Extract speed
        local speed=$(grep "eval time" "$output_file" 2>/dev/null | grep -oP '\d+\.\d+ tokens per second' | tail -1 || echo "N/A")
