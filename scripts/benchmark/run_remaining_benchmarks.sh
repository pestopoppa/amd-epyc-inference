...
check_existing() {
    ...
    if [[ -f "$RESULTS_CSV" ]]; then
        if grep -q ",${model},${method},.*,${prompt_type},success," "$RESULTS_CSV" 2>/dev/null; then
            local speed=$(grep ",${model},${method},.*,${prompt_type},success," "$RESULTS_CSV" | tail -1 | cut -d',' -f7)
...
run_lookup() {
    ...
    [[ -f "$model_path" ]] || { echo "Model not found: $model_path"; return 1; }
...
run_external_draft() {
    ...
    [[ -f "$model_path" ]] || { echo "Model not found: $model_path"; return 1; }
    [[ -f "$draft_path" ]] || { echo "Draft not found: $draft_path"; return 1; }
...
run_hard_mask() {
    ...
    local speed=$(echo "$output" | grep -oP '[\d.]+\s*tokens per second' | grep -oP '[\d.]+' | tail -1)
    if [[ -z "$speed" ]]; then
