...
run_memory_benchmark() {
    ...
    local baseline_kv=$(grep "CPU KV buffer size" "${OUTPUT_DIR}/${model_name}_baseline_memory.txt" 2>/dev/null | head -1 | sed 's/.*= *//')
    ...
    for block_size in "${BLOCK_SIZES[@]}"; do
        ...
        local paged_kv=$(grep "CPU KV buffer size" "${OUTPUT_DIR}/${model_name}_paged${block_size}_memory.txt" 2>/dev/null | head -1 | sed 's/.*= *//')
        local savings=$(grep "memory savings" "${OUTPUT_DIR}/${model_name}_paged${block_size}_memory.txt" 2>/dev/null | head -1 | grep -oE '[0-9.]+%')
