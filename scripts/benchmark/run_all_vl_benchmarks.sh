...
echo "=============================================="
echo ""
echo "Results in: /tmp/claude/vl_rubric_results/"
echo ""
...
for model_name in "${MODEL_ORDER[@]}"; do
    if [[ -v VL_MODELS[$model_name] ]]; then
        ...
        for cfg in "${configs[@]}"; do
            echo ""
            echo "=== $model_name ($cfg) ==="
            for f in /tmp/claude/vl_rubric_results/${model_name}_${cfg}_*.txt 2>/dev/null; do
                if [[ -f "$f" ]]; then
    ...
    if [[ -v VL_MODELS[$model_name] ]]; then
        ...
        if [[ "$arch" == "qwen3vlmoe" ]]; then
            ...
            for test in t1_q1_ocr t1_q2_shapes t1_q3_icon t2_q1_chart t2_q2_invoice t2_q3_code t3_q1_math t3_q2_flowchart t3_q3_diff t3_q4_puzzle; do
                base_file="/tmp/claude/vl_rubric_results/${model_name}_baseline_${test}.txt"
                moe_file="/tmp/claude/vl_rubric_results/${model_name}_moe4_${test}.txt"
