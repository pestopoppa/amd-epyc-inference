...
discover_models() {
    ...
    for dir in "${MODEL_DIRS[@]}"; do
        ...
        while IFS= read -r -d '' model_path; do
            ...
            local model_name=$(echo "$filename" | sed -E 's/\.(gguf|GGUF)$//' | sed -E 's/[-_](Q[0-9]_[A-Z0-9_]+|[Ff][0-9]+|BF16|bf16)$//')

            # Get architecture family
            local family=$(get_architecture_family "$model_name")
            ...
            # Categorize by size
            local is_draft=$(echo "$size_gb < $MAX_DRAFT_SIZE_GB" | bc -l)
            local is_target=$(echo "$size_gb >= $MIN_TARGET_SIZE_GB" | bc -l)
...
test_acceptance_rate() {
    ...
    local acceptance=$(grep -oP 'draft acceptance rate: \K[\d.]+' "$output_file" 2>/dev/null | tail -1 || echo "0")
...
run_optimization_sweep() {
    ...
    local baseline_speed=$(grep "eval time" "$baseline_file" 2>/dev/null | grep -oP '[\d.]+(?= tokens per second)' | tail -1 || echo "0")
    ...
    for k in "${K_VALUES[@]}"; do
        ...
        local speed=$(grep "eval time" "$test_file" 2>/dev/null | grep -oP '[\d.]+(?= tokens per second)' | tail -1 || echo "0")
        log "        K=$k -> ${speed} t/s"
    ...
    if [[ "$USE_TEMP_BINARY_SEARCH" == "true" ]] && [[ "$QUICK_MODE" != "true" ]]; then
        ...
        while (( $(echo "$high - $low > $TEMP_PRECISION" | bc -l) )) && [[ $iteration -lt $max_iterations ]]; do
            ...
            local mid=$(echo "scale=6; ($low + $high) / 2" | bc)
            local q1=$(echo "scale=6; ($low + $mid) / 2" | bc)
            local q3=$(echo "scale=6; ($mid + $high) / 2" | bc)

            log "        Iteration $iteration: testing temps [$(printf '%.3f' $low), $(printf '%.3f' $q1), $(printf '%.3f' $mid), $(printf '%.3f' $q3), $(printf '%.3f' $high)]"

            local speed_low=$(test_temp_spec "$low")
            local speed_q1=$(test_temp_spec "$q1")
            local speed_mid=$(test_temp_spec "$mid")
            local speed_q3=$(test_temp_spec "$q3")
            local speed_high=$(test_temp_spec "$high")
    ...
    local best_file="$pair_dir/k${best_k}_temp$(printf '%.4f' $best_temp).txt"
    if [[ -f "$best_file" ]]; then
...
EOF
    ...
    for target_path in "${!TARGET_MODELS[@]}"; do
        ...
        for draft_path in "${!DRAFT_MODELS[@]}"; do
            ...
            local acceptance=$(test_acceptance_rate "$target_path" "$draft_path" "$acceptance_file")
            ...
            local sweep_result=$(run_optimization_sweep "$target_path" "$target_name" "$draft_path" "$draft_name" "$pair_dir")
